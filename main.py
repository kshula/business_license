...
if data:
    ...
    with col1:
        ...
        fig_license_count = px.bar(
            license_count, x=license_count.index, y=license_count.values,
        ...
        fig_revenue_description = px.bar(
            revenue_by_description, x='Year', y='Amount', color='Description', barmode='stack',
    ...
    with col2:
        ...
        fig_total_revenue = px.bar(
            total_revenue, x=total_revenue.index, y=total_revenue.values,
        ...
        fig_trend_desc = px.line(
            combined_data, x="Year", y="Amount", color="Description",
    ...
    knn_fig = px.scatter(
        x=knn_pca[:, 0], y=knn_pca[:, 1], color=combined_data['Description'],
    ...
    with col4:
        ...
        fig_revenue_segment = px.bar(
            revenue_segment_summary, title="Revenue Segmentation Over Time by Description"
        )
    ...
    st.download_button(
        "Download Combined Data as CSV", data=csv_data, file_name="combined_data.csv", mime="text/csv"
    )
